...
namespace mersolutionCore.Command.MySQL
{
    ...
    public class MySqlCommand : DbCommandBase
    {
        ...
        private MySqlConnector.MySqlCommand _mysqlCommand;
        ...
        protected override void CreateConnection()
        {
            ...
            _mysqlConnection = new MySqlConnection(connStr);
        ...
        {
            _mysqlCommand = new MySqlConnector.MySqlCommand();
            _mysqlCommand.Connection = _mysqlConnection;
            _mysqlCommand.CommandTimeout = _config.Timeout;
            Command = _mysqlCommand;
        }

        protected override void CreateDataAdapter()
        {
            _mysqlDataAdapter = new MySqlDataAdapter(_mysqlCommand);
            DataAdapter = _mysqlDataAdapter;
        }
        ...
        {
            if (_mysqlCommand == null) CreateCommand();
            _mysqlCommand.Parameters.Add(name, mysqlType).Value = value ?? DBNull.Value;
        }
        ...
        {
            if (_mysqlCommand == null) CreateCommand();
            _mysqlCommand.Parameters.Add(name, mysqlType, size).Value = value ?? DBNull.Value;
        }
        ...
        private void ApplyPendingParameters()
        {
            ...
            {
                _mysqlCommand.Parameters.AddWithValue(param.Key, param.Value);
            }
        ...
        public void RunExecuteStoredProcedure(string procedureName)
        {
            try
            {
                CreateConnection();
                CreateCommand();
                _mysqlCommand.CommandText = procedureName;
        ...
        /// <summary>
        /// Execute SQL query with pagination (MySQL LIMIT OFFSET syntax)
        /// </summary>
        ...
        public void RunDataReaderStreaming(string sql, Action<MySqlDataReader> action)
        {
            try
            {
                ...
                using (var reader = _mysqlCommand.ExecuteReader(CommandBehavior.SequentialAccess))
                {
                    while (reader.Read())
                    {
                        action(reader);
